 ...
 *
 * This decisioner figures out which bot deserves it most (or is most vulnerable) and goes after
 * them.
 *
 * On
 */
public class BotTargetingDecisioner implements Decision<AdvancedMurderBot.GameContext, BotMove> {
  ...
  public BotTargetingDecisioner(
      Decision<AdvancedMurderBot.GameContext, BotMove> noTargetFoundDecisioner) {
  ...
  public BotMove makeDecision(AdvancedMurderBot.GameContext context) {
    ...
    for (GameState.Hero currentHero : heroesWithMines) {
      ...
      if (currentHero.getName().equals("don") || currentHero.getName().equals("rauflala")
          || currentHero.getName().equals("go4gold"))
        continue;
      ...
      if (currentHero.getLife() > 10 && currentHero.getLife() > me.getLife())
        continue;
